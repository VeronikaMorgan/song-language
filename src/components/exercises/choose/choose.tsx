...
import { useAnswers } from "../../../utils/hooks/use-answers-hook";
import { shuffleArray } from "../../../utils/helpers";
//add uuid
const ChooseExercise: FC<{ data: TChooseExerciseData[], color: string }> = ({ data, color }) => {
  const qwty = data.length;
  const [shuffledData, setShuffledData] = useState<TChooseExerciseData[]>()
  const { answers, handleSetAnswer, setAnswers } = useAnswers()
  ...
  const handleCheck = (e: React.MouseEvent) => {
    ...
    const isCorrect = shuffledData.every((item, i) => item.rightOption === answers[i])
  ...
  return (
    <div className={styles.wrapper}>
      <motion.div animate={controls} className={`${styles.title} text text_last`}>{checkAnswers ? (allCorrect ? `Tutto e Coretto! \u{1F355}` : 'Ben fatto, non arrenderti' ): 'Trova il significato delle seguenti parole'}</motion.div>
      <ul className={`${styles.exercise} list-default`}>
        {
          shuffledData?.map((item, i) => (
            <li key={i} className={styles.item}>
              <ChooseExerciseItem color={color} check={checkAnswers} show={showAnswers} data={item} setAnswer={handleSetAnswer} taskIndex={i} chosenAnswer={answers[i]} />
            </li>
          ))
        }</ul>
      {
        checkAnswers ?
          <div className={styles.btns}>
            <motion.button className='text_type_large' onClick={handleShow} style={{ fontWeight: 500 }} whileHover={{ scale: 1.05, transition: { duration: .7, type: 'spring', bounce: .7 } }}>Guardare i soluzioni</motion.button>
            <motion.button  onClick={handleRetry} animate={returnBtnControls} className={styles.retry} whileHover={{ rotate: -360, scale: 1.1, transition: { duration: .3 } }}>
          ...
          :
          <motion.button className={styles.check} onClick={handleCheck} style={{ backgroundColor: `var(${color})`, }} whileHover={{ scale: 1.1, transition: { duration: .7, type: 'spring', bounce: .7 } }}>Provare</motion.button>
      }
    </div>
  )
}
