...
//add uuid
const JoinExercise: FC<{ data: TJoinExerciseData, color: string }> = ({ data, color }) => {
  ...
  return (
    <div className={styles.wrapper}>
      <motion.div animate={controls} className={`${styles.title} text text_last`}>{correct ? `Tutto e Coretto! \u{1F355}` : 'Abbina le parole ai loro significati'}</motion.div>
      <div className={styles.exercise}>
        <ul className={`${styles.items} list-default`}>
          {
            keys && keys.map((key, i) => (
              <li key={i} className={styles.item} style={{ borderColor: `var(${color})` }}>{key}</li>
            ))
          }</ul>
        <Reorder.Group axis="y" values={shuffledValues} onReorder={setShuffledValues} className={`${styles.items} list-default`}>
          <AnimatePresence>
            {shuffledValues.map((value) => (
              <Reorder.Item
                whileDrag={{ opacity: .6 }}
                key={value} animate={controls} value={value} className={styles.item} style={{ backgroundColor: correct ? `var(${color})` : '', borderColor: `var(${color})` }}>
                {value}
              </Reorder.Item>
            ))}
          </AnimatePresence>
      ...
      <motion.button disabled={!correct} animate={returnBtnControls} className={styles.retry} onClick={handleReshuffle} whileHover={{ rotate: -360, scale: 1.1, transition: { duration: .3 } }}><svg width="40" height="40" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
